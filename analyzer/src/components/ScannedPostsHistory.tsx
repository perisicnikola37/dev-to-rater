...
const ScannedPostsHistory: React.FC<ScannedPostsHistoryProps> = ({
  ...
  if (history.length === 0) {
    return (
      <div className="history-section mt-2 max-w-3xl mx-auto px-4 text-gray-500 dark:text-white">
        <div className="text-center text-xl">
          <p>
            We've scanned:&nbsp;
            <span className="text-blue-500 text-2xl">{counter}</span> posts
          </p>
        </div>
      </div>
    )
  ...
  return (
    <div className="history-section mt-8 max-w-3xl mx-auto px-4">
      <div className="flex justify-between items-center mb-6">
        ...
        <button
          className="border-2 border-red-300 dark:border-red-400 cursor-pointer py-2 px-4 rounded-md hover:bg-[#F56565] transition duration-300 hover:text-white outline-none hover:border-2 hover:border-transparent"
        ...
        {history.slice(0, MAX_SCANNED_VISIBLE_POSTS).map((post, index) => (
          ...
          >
            ...
            <div className="post-details flex flex-col">
              <h4 className="post-title text-lg font-medium text-gray-800 dark:text-white">
                {post.title}
              </h4>
              <p className="text-sm text-gray-500 dark:text-blue-400 truncate max-w-xs">
                {post.imageUrl}
              </p>
              <div className="flex space-x-2 mt-2 text-black dark:text-white">
